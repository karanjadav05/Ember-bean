...
export const CoffeeCanvas: React.FC<CoffeeCanvasProps> = ({ progress }) => {
  ...
  // Keep progressRef updated without restarting the canvas animation loop
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const width = Math.max(canvas.clientWidth || window.innerWidth || 800, 100);
      const height = Math.max(canvas.clientHeight || window.innerHeight || 600, 100);
    ...
    const render = () => {
      ...
      // Verify canvas resolution buffer matches container
      ...
      try {
        ...
        const beamRadius = Math.max(10, Math.max(width, height) * 0.85);
        const sunbeamGrad = ctx.createRadialGradient(
          sunbeamX, sunbeamY, 1,
          sunbeamX, sunbeamY, beamRadius
        );
        ...
        if (p < 0.12) {
          ...
          if (s1Opacity > 0.01) {
            ...
            const glowGrad = ctx.createRadialGradient(cx, cy + 20, 1, cx, cy + 20, 120);
            ...
            for (let s = 0; s < 3; s++) {
              ...
              ctx.lineWidth = Math.max(0.5, 2 - st * 0.5);
              ctx.beginPath();
        ...
        // SCENE 02: THE BEANS (p: 0.05 -> 0.38)
        if (p > 0.05 && p < 0.38) {
          ...
          beansData.forEach((b) => {
            ...
            if (bOpacity > 0.01) {
              ...
              const beanGrad = ctx.createRadialGradient(-3, -3, 1, 0, 0, 16);
        ...
        // SCENE 03: THE GRIND (p: 0.22 -> 0.48)
        if (p > 0.22 && p < 0.48) {
        ...
        // SCENE 04 & 05: THE BLOOM & THE POUR (p: 0.38 -> 0.75)
        if (p > 0.38 && p < 0.75) {
          ...
          if (bOpacity > 0.01) {
            ...
            const bedHeight = Math.max(1, 25 + bloomFactor * 12);

            ctx.fillStyle = '#21140E';
            ctx.beginPath();
            ctx.ellipse(dx, dy - 20, Math.max(1, 65 + bloomFactor * 5), bedHeight, 0, 0, Math.PI * 2);
            ctx.fill();
        ...
        // SCENE 06, 07, 08: THE WAIT, READY CUP & SWEET MESSAGE (p: 0.68 -> 1.00)
        if (p > 0.68) {
          ...
          const shadowGrad = ctx.createRadialGradient(0, 70, 1, 0, 70, 140);
          ...
          const liquidGrad = ctx.createRadialGradient(-15, -10, 1, 0, 0, 85);
          ...
          const rawRippleR = ((time * 30) % 60);
          const rippleR = Math.max(0.5, rawRippleR);
          const rippleAlpha = 1 - rawRippleR / 60;
          ctx.strokeStyle = `rgba(232, 216, 189, ${rippleAlpha * 0.35})`;
          ctx.lineWidth = 1.5;
          ctx.beginPath();
          ctx.ellipse(0, 0, Math.max(0.5, rippleR * 1.2), Math.max(0.5, rippleR * 0.8), 0, 0, Math.PI * 2);
          ctx.stroke();
          ...
          for (let s = 0; s < 4; s++) {
            ...
            ctx.lineWidth = Math.max(0.5, 3 - steamT * 0.5);
            ctx.beginPath();
